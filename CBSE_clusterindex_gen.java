...
class CBSE_clusterindex_gen
{
	public static void main(String args[])
	{
		try
		{
			cluster_downer ob=new cluster_downer();
			ob.cal_index();
		}catch(Exception e){System.out.println(e.toString());		}
	}
}

class cluster_downer
{
...
public void cal_index()throws Exception
{
	...
	String query="select * from `cluster_keywords`  where `Sr`>0";
	...
	while(rs.next())
	{
		...
		String query1="insert into `cluster_index`(`cid`,`Cluster_Index`,`time`) values('"+cid+"','"+final_keyword+"','"+x+"');";
